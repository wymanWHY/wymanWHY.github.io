...
> 在for 后面的双括号中，给循环变量 n 赋了一个初值 1，当 n 小于或者等于 10 的时候，就把 sum 和 n 的和赋值给变量 sum ，然后 n 再加一。刚开始的时候，n=1，sum=0，这个时候 n 的值是小于 10 的，所以进入循环体，执行 sum+=n ，这个时候 sum 的值就变成了 1 ，然后执行 n++ ，这个时候 n 的值变为了 2，此时 n 的值还是小于 10 的，所以再次进入循环体，依次类推。直到 n 的值大于 10 时，就不再进入循环体。
